	...
	protected Dialog onCreateDialog(int id) {
		...
		switch (id) {
			...
			case EXTERNAL_STORAGE_UNAVAILABLE:
				dialog = new AlertDialog.Builder(this).setTitle(R.string.external_storage_na_title).setMessage(R.string.external_storage_na_message).create();
				break;
		}
		return dialog;
	}
...
//			Dialog dialog = new AlertDialog.Builder(this).setTitle("HiRes").setMessage("A low-resolution image was created. Do you want to save the hi-res image too?").setPositiveButton(android.R.string.yes, noListener).setNegativeButton(android.R.string.no, noListener).setCancelable(false).create();
...
//			dialog.show();
	...
	private Uri pickRandomImage() {
		Cursor c = getContentResolver().query(Images.Media.EXTERNAL_CONTENT_URI, new String[]{ImageColumns._ID, ImageColumns.DATA}, ImageColumns.SIZE + ">?", new String[]{"90000"}, null);
		Uri uri = null;
	...
	private File getNextFileName() {
		if (mGalleryFolder != null) {
			if (mGalleryFolder.exists()) {
				File file = new File(mGalleryFolder, "Gagan_" + System.currentTimeMillis() + ".jpg");
				return file;
			}
		}
		return null;
...
//		Log.d(LOG_TAG, "uri: " + uri);
	...
	private void processHD(final String session_name) {
		...
		try {
			sessionUri = FeatherContentProvider.SessionsDbColumns.getContentUri(null, session_name);
			cursor = getContentResolver().query(sessionUri, null, null, null, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (null != session) {
			...
			Uri actionsUri = FeatherContentProvider.ActionsDbColumns.getContentUri(null, session.session);
	...
	private void deleteSession(final String session_id) {
		...
		try {
			Uri uri = FeatherContentProvider.SessionsDbColumns.getContentUri(null, session_id);
	...
	private class HDAsyncTask extends AsyncTask<Cursor, Integer, String> {
		...
		protected void onPostExecute(String errorString) {
			...
			new AlertDialog.Builder(MainActivity.this).setTitle("File saved").setMessage("File saved in " + dstPath_ + ". Do you want to see the HD file?").setPositiveButton(android.R.string.yes, new OnClickListener() {
