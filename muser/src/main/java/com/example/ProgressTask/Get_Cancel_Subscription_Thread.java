...
public class Get_Cancel_Subscription_Thread
{
	Context con;
	SharedPreferences rem_pref;
	
	public Get_Cancel_Subscription_Thread(Context con)
	{
		this.con = con;
	...
	Thread background = new Thread(new Runnable()
	{
		...
		public void run()
		{
			try
			{
				HttpPost httppost = new HttpPost(Util_Class.cancel_subscription);
				...
				if((null != response))
				{
					try
					{
						...
						if(msg_json.equals("Success"))
						{
							...
							((Cancel_Sessions) con).set_data(list);
							
						
						}
						else if(msg_json.equals("Failure"))
						{
							((Cancel_Sessions) con).on_Failure();
						}
					}
