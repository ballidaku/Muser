...
public class Get_Funds_Thread
{
	Context con;
	SharedPreferences rem_pref;
	Handler handler_trainer;
	
	public Get_Funds_Thread(Context con, Handler handler)
	{
		this.con = con;
		this.handler_trainer=handler;
		rem_pref = con.getSharedPreferences("Remember", con.MODE_WORLD_READABLE);
		background.start();
	}

	public Get_Funds_Thread(Context con)
	{
		this.con = con;
		rem_pref = con.getSharedPreferences("Remember", con.MODE_WORLD_READABLE);
		background.start();
	...
	Thread background = new Thread(new Runnable()
	{
		...
		public void run()
		{
			try
			{
				HttpPost httppost = new HttpPost(Util_Class.get_funds);
			...
			public void handleMessage(Message msg)
			{
				...
				if((null != response))
				{
					try
					{
						...
						{
							if(con instanceof Drawer)
							{
								String balance = new JSONObject(response).getString("balance");
								Wallet.set_balance(balance);
							}
							else //if(con instanceof Trainers)
							{
								String balance = new JSONObject(response).getString("balance");
//								((Trainers) con).set_balance(balance);
								...
								handler_trainer.sendMessage(msg1);
							}
						}
					}
