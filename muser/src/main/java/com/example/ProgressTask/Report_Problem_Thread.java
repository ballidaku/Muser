...
public class Report_Problem_Thread
{
	Context con;
	String subject,message;
	SharedPreferences rem_pref;

	public Report_Problem_Thread(Context con,String subject ,String message)
	{
	...
	Thread background = new Thread(new Runnable()
	{
		...
		public void run()
		{
			try
			{
				HttpPost httppost = new HttpPost(Util_Class.report_problem);
		...
		private final Handler handler = new Handler()
		{
			...
			public void handleMessage(Message msg)
			{
				...
				if((null != response))
				{
					try
					{
						String msg_json = new JSONObject(response).getString("message");
						if(msg_json.equals("Success"))
						{
							Util_Class.show_Toast("Problem sent successfully.", con);
