...
public class Get_Wallet_Trainers_Thread
{
	Context con;
	SharedPreferences rem_pref;
	
	public Get_Wallet_Trainers_Thread(Context con)
	{
	...
	Thread background = new Thread(new Runnable()
	{
		...
		public void run()
		{
			try
			{
				HttpPost httppost = new HttpPost(Util_Class.withdraw_get_trainers);
			...
			public void handleMessage(Message msg)
			{
				...
				if((null != response))
				{
					try
					{
						...
						{
							JSONArray jo 	= new JSONObject(response).getJSONArray("tainers_info");
							...
							for(int i = 0; i < jo.length(); i++)
							{
								...
								map.put("is_subscribed", obj.getString("is_subscribed"));
								
								
								list.add(map);
							}
							
							((Trainers) con).set_data(list);
							
							
						}
						else if(msg_json.equals("Failure"))
						{
							((Trainers) con).on_Failure();
						}
