...
public class Send_Invite_Contacts_Thread
{
	Context con;
	String numbers,contry_code;
	SharedPreferences rem_pref;
	Fragment frag;

	public Send_Invite_Contacts_Thread(Context con,Fragment frag,String numbers,String contry_code)
	{
	...
	Thread background = new Thread(new Runnable()
	{
		...
		public void run()
		{
			try
			{
				HttpPost httppost = new HttpPost(Util_Class.send_invite_contact);
				DefaultHttpClient httpclient = new DefaultHttpClient();
				
				List<NameValuePair> param = new ArrayList<NameValuePair>();
				param.add(new BasicNameValuePair("user_id", rem_pref.getString("user_id", "")));
				param.add(new BasicNameValuePair("phones", numbers));
				param.add(new BasicNameValuePair("country_code", contry_code));
				...
				if((null != response))
				{
					try
					{
						...
						if(msg_json.equals("Success"))
						{
							...
							((Tab_Invite_Others_Contacts)frag).refresh_checkbox();
